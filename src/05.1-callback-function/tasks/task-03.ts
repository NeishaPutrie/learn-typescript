...
function processProducts(products: { name: string; price: number }[], 
...
processProducts(products, (p) => {
    if (p.price > 500000) {
        const discount = p.price * 0.9;
        console.log(`${p.name} - Rp${p.price} -> Rp${discount}`);
    }
});
